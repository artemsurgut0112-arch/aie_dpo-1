...
class QualityFlagsResponse(BaseModel):
    """Ответ эндпоинта /quality-flags-from-csv с полным набором флагов качества."""

    flags: dict[str, bool | float] = Field(
        ...,
        description="Полный набор флагов качества данных с подробными эвристиками",
    )
    latency_ms: float = Field(
        ...,
        ge=0.0,
...
class SummaryResponse(BaseModel):
    """Ответ эндпоинта /summary-from-csv: JSON-сводка датасета."""

    summary: dict[str, Any] = Field(
        ...,
        description="Сводка датасета в формате JSON (как --json-summary CLI)",
    )
    latency_ms: float = Field(
        ...,
        ge=0.0,
...
# ---------- /quality-flags-from-csv: полный набор флагов по CSV ----------
...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    ...
    Эндпоинт, который принимает CSV-файл, запускает EDA-ядро
    (summarize_dataset + missing_table + compute_quality_flags)
    и возвращает полный набор флагов качества, включая все эвристики
    (константные колонки, высокая кардинальность, дубликаты ID, много нулей и т.д.).
    """
...
# ---------- /summary-from-csv: JSON-сводка по CSV (как --json-summary CLI) ----------
...
@app.post(
    ...
    tags=["quality"],
    summary="JSON-сводка датасета по CSV-файлу",
)
async def summary_from_csv(file: UploadFile = File(...)) -> SummaryResponse:
    """
    Эндпоинт, который принимает CSV-файл, строит сводку через `summarize_dataset`
    и возвращает её в JSON (аналог опции CLI --json-summary).
    """
    ...
    return SummaryResponse(summary=summary.to_dict(), latency_ms=latency_ms)